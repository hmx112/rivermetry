...
def discover_usgs_candidates(client: httpx.Client, limit: int = 450, api_key: str | None = None) -> list[dict]:
    params = {"f": "json", "limit": "10000"}
    if api_key:
        params["api_key"] = api_key
    try:
        response = client.get(LATEST_URL, params=params, headers={"User-Agent": "Rivermetry/0.1 (+https://rivermetry.example)"}, timeout=60)
    ...
    for feature in features:
        ...
        if not sid:
            continue
        try:
            observed = datetime.fromisoformat(str(p.get("time") or "").replace("Z", "+00:00"))
    ...
    for station in stations.values():
        ...
        data_quality = 35 if station["age_minutes"] <= 30 else 25 if station["age_minutes"] <= 90 else 10
    ...
    qualified.sort(key=lambda x: (-x["data_quality_score"], -x["demand_score"], x["station_id"]))

    # Round-robin states to prevent one dense state from consuming the pool.
