...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command in {"build", "refresh-static"}:
            fixture = args.command == "build" and args.fixtures
            build_site(args.output, fixture_mode=fixture, registry_path=args.registry)
            errors = guard_output(args.output)
            if errors:
        ...
        if args.command == "discover-usgs":
            with httpx.Client(follow_redirects=True) as client:
                candidates = discover_usgs_candidates(client, args.limit, os.environ.get("USGS_API_KEY"))
